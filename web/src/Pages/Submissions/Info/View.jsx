...
function regrade(submissionId, enqueueSnackbar) {
  axios
    .get(`/api/public/regrade/${submissionId}`)
    .then(response => {
...
export default function SubmissionInfo() {
  ...
  const {loading, error, data} = useSubscribe(
    `/api/public/submission/${useQuery().get('submission')}`,
    1000,
  ...
  function translateSubmission(
    {
      assignment_name, assignment_due, url, id,
      processed, state, created, tests, build, class_code
    }) {
    return {
      ...
      dateSubmitted: created.split(' ')[1], timeStamp: new Date(created), submissionId: id,
                    ...
                    <ListItemIcon>
                      <Tooltip title={!submission.processed ? submission.submissionState : "regrade"}>
                        <IconButton component="div" onClick={() =>
                          regrade(submission.submissionId, enqueueSnackbar)}>
